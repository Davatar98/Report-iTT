...
class AverageAcknowledgeTimePerFaultChart extends BaseChart
{
    /**
     ...
     */
    public ?string $name = 'acknowledgement_time_average';

    public ?string $routeName = 'acknowledgement_time_average';
    ...
    public function handler(Request $request): Chartisan
    {
         ...
         foreach ($faults as $fault){
             ...
             // each time the report status is updated(using a job or event). This would eliminate the query below.
              ...
              array_push($faultCount1, round(Report::where('title',$fault)->avg('acknowledgement_time_hrs'),2) );
         }
           return Chartisan::build()
               ->labels($faultName1)
               ->dataset('Average Acknowledgement Time', $faultCount1);
